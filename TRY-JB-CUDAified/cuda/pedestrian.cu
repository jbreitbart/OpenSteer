...
#if 0
const float3 float3_zero    = make_float3 (0.0f, 0.0f, 0.0f);
const float3 float3_up      = make_float3 (0.0f, 1.0f, 0.0f);
const float3 float3_forward = make_float3 (0.0f, 0.0f, 1.0f);
#endif

#if 0
__device__ float3 globalizePosition (
...
{
	return position + globalizeDirection (localPosition, side, up, forward);
};
#endif
	...
	)
{
	...
	float3* device_result;
	
	cudaMalloc ((void**)(&device_result), crowd_size*sizeof(float3));

	
	// vehicle data	
	float3* device_vehicle_forward = 0;
	float3* device_vehicle_position = 0;
	float2* device_vehicle_random = 0;
	float * device_vehicle_radius = 0;
	float3* device_vehicle_side = 0;
	float3* device_vehicle_up = 0;
	float * device_vehicle_speed = 0;
	float * device_vehicle_max_force = 0;
	float * device_vehicle_max_speed = 0;
	float4* device_vehicle_wander_rand = 0;
	int   * device_vehicle_pathDirection = 0;
	...
	//data needed for rectangle objects
	float3* device_object_rectangle_position = 0;
	float3* device_object_rectangle_side = 0;
	float3* device_object_rectangle_up = 0;
	float3* device_object_rectangle_forward = 0;
	int*    device_object_rectangle_seen_from = 0;
	float*  device_object_rectangle_width = 0;
	float*  device_object_rectangle_height = 0;
	...
	//data needed for sphere objects
	float3* device_object_sphere_center = 0;
	float*  device_object_sphere_radius = 0;
	int*    device_object_sphere_seen_from = 0;
	...
	float3* device_path_points = 0;
	float3* device_path_segmentTangents = 0;
	float * device_path_segmentLengths = 0;
	...
	// start the kernel
	determineCombinedSteering_kernel <<< grid_dim, block_dim >>> (
	        elapsedTime,
