...
#include "../kapaga/pedestrian_state.h"
...
__global__ void determineCombinedSteering_kernel_op_2 (
                   const float elapsedTime,
                   const unsigned int crowd_size,

                   // vehicle data
                   const pedestrian_state* const p_states,
	...
	)
{
	...
	const pedestrian_state p_state = p_states[index];
	
	// move forward
	float3 steeringForce = p_states[index].vehicle_forward;

	// probability that a lower priority behavior will be given a
	// chance to "drive" even if a higher priority behavior might
	// otherwise be triggered.
	const float  &random0                     = p_state.vehicle_random.x;
	const float  &random1                     = p_state.vehicle_random.y;
	const float3 &local_vehicle_forward       = p_state.vehicle_forward;
	const float3 &local_vehicle_position      = p_state.vehicle_position;
	const float  &local_vehicle_radius        = p_state.vehicle_radius;
	const float3 &local_vehicle_side          = p_state.vehicle_side;
	const float3 &local_vehicle_up            = p_state.vehicle_up;
	const float  &local_vehicle_speed         = p_state.vehicle_speed;
	const float  &local_vehicle_max_force     = p_state.vehicle_max_force;
	const float  &local_vehicle_max_speed     = p_state.vehicle_max_speed;
	const float3 local_vehicle_velocity       = local_vehicle_forward * local_vehicle_speed;
	const float4 &local_vehicle_wander_rand   = p_state.vehicle_wander_rand;
	const int    &local_vehicle_pathDirection = p_state.vehicle_pathDirection;
	...
	if (obstacleAvoidance == make_float3 (0.0f, 0.0f, 0.0f) ) {
		...
		if (leakThrough < random1 ) {
			...
			for (int i=0; i<crowd_size; ++i) {
				if (i==index) continue;
				const float3 offset = local_vehicle_position - p_states[i].vehicle_position;
				...
				if (d2<r2) {
					if ( neighbors_size < neighbor_size_max ) {
						...
						neighbor_radius[neighbors_size]   = p_states[i].vehicle_radius;
						neighbor_position[neighbors_size] = p_states[i].vehicle_position;
						neighbor_speed[neighbors_size]    = p_states[i].vehicle_speed;
						neighbor_forward[neighbors_size]  = p_states[i].vehicle_forward;
						neighbor_velocity[neighbors_size] = p_states[i].vehicle_forward * p_states[i].vehicle_speed;
						++neighbors_size;
					} else {
						
						if ( d2 < max_neighbour_distance ) {
							neighbor_radius[max_neighbour_distance_index]   = p_states[i].vehicle_radius;
							neighbor_position[max_neighbour_distance_index] = p_states[i].vehicle_position;
							neighbor_speed[max_neighbour_distance_index]    = p_states[i].vehicle_speed;
							neighbor_forward[max_neighbour_distance_index]  = p_states[i].vehicle_forward;
							neighbor_velocity[max_neighbour_distance_index] = p_states[i].vehicle_forward * p_states[i].vehicle_speed;
...
extern "C"
void GPU_determineCombinedSteering_op_2 (
		const float elapsedTime,
	...
	)
{
	...
	// vehicle data
	static pedestrian_state* device_p_states = 0;
	...
	if (currently_allocated_vehicles != crowd_size) {
		...
		cudaFree (device_p_states);
		
		cudaMalloc ((void**)(&device_result), crowd_size*sizeof(float3));
		cudaMalloc ((void**)(&device_p_states), crowd_size*sizeof(pedestrian_state));
	...
	cudaMemcpy (device_p_states, p_states, crowd_size*sizeof(pedestrian_state), cudaMemcpyHostToDevice);
	...
	// start the kernel
	determineCombinedSteering_kernel_op_2 <<< grid_dim, block_dim >>> (
	        elapsedTime,
