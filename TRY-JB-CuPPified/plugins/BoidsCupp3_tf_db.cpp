...
class BoidsPlugIn : public PlugIn {
	private:
		...
		cupp::vector< Matrix > render_position_[2];
		int render_position_index_;
	...
	public:
		...
		const char* name (void) {return "BoidsCuPP inkl. simulate inkl. update with thinking frequency and double buffering";}
		float selectionOrderSortKey (void) {return 0.03f;}

		BoidsPlugIn() :
		find_neighbours(get_find_neighbours_simulate_frequency_kernel(), dim3 (threads_per_block/threads_per_block), dim3 (threads_per_block)),
		...
		virtual void update (const float /*currentTime*/, const float elapsedTime) {
			...
			render_position_[(render_position_index_+1)%2].update_host();

			//render_position_.host_changes_ = false;
			if (flock_size != steering_result_.size()) {
				steering_result_.resize(flock_size);
				render_position_[0].resize(flock_size, Matrix(Vec3(), Vec3(0, 1, 0), Vec3(), Vec3()) );
				render_position_[1].resize(flock_size, Matrix(Vec3(), Vec3(0, 1, 0), Vec3(), Vec3()) );
			...
			find_neighbours (cupp_device, think_lower_boid_index, flock_size, positions_, forwards_, steering_result_);

			update_flock(cupp_device, elapsedTime, positions_, forwards_, steering_result_, flock, render_position_[render_position_index_]);
			
			//std::cout << "calculate " << render_position_index_ << std::endl;
		...
		virtual void redraw (const float /*currentTime*/, const float /*elapsedTime*/) {
			...
			render_position_index_ = (render_position_index_+1)%2;
			
			for (std::size_t i=0; i<render_position_[render_position_index_].size(); ++i) {
				renderFeeder() -> render (render_position_[render_position_index_][i], boidGraphicsId_[i%3]);
			}
			//std::cout << "draw " << render_position_index_ << std::endl;
