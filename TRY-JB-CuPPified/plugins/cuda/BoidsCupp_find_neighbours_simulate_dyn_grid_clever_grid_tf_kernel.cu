...
__device__ __constant__ int grid_index_mix[grid_nb_of_cells+1];

int* get_grid_index_symbol_mix() {
	return grid_index_mix;
}
...
__global__ void find_neighbours_simulate_dyn_gr_clever_tf (
                                                 const ds::deviceT::dyn_grid_clever    &think_grid_,
                                                 const ds::deviceT::grid<int>          &rest_grid_,
...
{
	__shared__ ds::deviceT::dyn_grid_clever think_grid;
	__shared__ ds::deviceT::grid<int>       rest_grid;
	...
	__shared__ int3 rest_grid_start_index;
	__shared__ int3 rest_grid_end_index;
	
	if (threadIdx.x == 0) {
		think_grid = think_grid_;
		rest_grid = rest_grid_;
		cur_block = think_grid.get_block_data(blockIdx.x);
		positions = positions_;
		forwards = forwards_;
		my_grid_start_index = rest_grid.get_cell_index( Vec3(cur_block.low_x, cur_block.low_y, cur_block.low_z) );
		my_grid_end_index =   rest_grid.get_cell_index( Vec3(cur_block.high_x, cur_block.high_y, cur_block.high_z) );
	}
	...
	if (threadIdx.x < cur_block.size) {
		my_index = think_grid.get_index(cur_block, threadIdx.x);
		
		position = think_grid.get_position(cur_block, threadIdx.x);
	...
	// think grid
	{
		for (int i=0; i<gridDim.x; i+=threads_per_block) {
			...
			if (i+threadIdx.x < gridDim.x) {
				s_leaves[threadIdx.x] = think_grid.get_block_data (i+threadIdx.x);
			...
			for (int j=0; j<threads_per_block; ++j) {
				if (i+j<gridDim.x && s_useful[j]) {
					neighbour_search(think_grid, s_leaves[j], cur_block, my_index, position, neighbours_found, neighbours, neighbours_distance_squared);
				}
			}
	...
	// rest grid
	{
		for (int x = rest_grid_start_index.x; x <= rest_grid_end_index.x; ++x) {
			for (int y = rest_grid_start_index.y; y <= rest_grid_end_index.y; ++y) {
				for (int z = rest_grid_start_index.z; z <= rest_grid_end_index.z; ++z) {

					const int3 cell = make_int3(x, y, z);
		...
		const int rest_grid_size = rest_grid.size();
		for (int i=0; i<rest_grid_size; i+=threads_per_block) {

			if (i+threadIdx.x < gridDim.x) {
				s_leaves[threadIdx.x] = rest_grid.get_block_data (i+threadIdx.x);
				s_useful[threadIdx.x] = in_range(s_leaves[threadIdx.x], cur_block);
			}
			__syncthreads();
			...
			for (int j=0; j<threads_per_block; ++j) {
				if (i+j<gridDim.x && s_useful[j]) {
					neighbour_search(rest_grid, s_leaves[j], cur_block, my_index, position, neighbours_found, neighbours, neighbours_distance_squared);
				}
			}
...
find_neighbours_simulate_dyn_gr__clever_tf_kernelT get_find_neighbours_simulate_dyn_grid_clever_tf_kernel() {
	return (find_neighbours_simulate_dyn_gr__clever_tf_kernelT)find_neighbours_simulate_dyn_gr_clever_tf;
}
