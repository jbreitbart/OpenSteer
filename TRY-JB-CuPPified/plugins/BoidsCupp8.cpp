...
class BoidsPlugIn : public PlugIn {
	...
	public:
		...
		const char* name (void) {return "BoidsCuPP inkl. simulate inkl. update with dyn_grid";}
		float selectionOrderSortKey (void) {return 0.03f;}

		BoidsPlugIn() :
		find_neighbours(get_find_neighbours_simulate_dyn_grid_kernel(), dim3 (threads_per_block/threads_per_block), dim3 (threads_per_block)),
