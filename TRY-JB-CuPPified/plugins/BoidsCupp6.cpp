...
class BoidsPlugIn : public PlugIn {
	private:
		...
		cupp::kernel preprocess_grid;

		ds::gpu_grid grid_;
	public:

		const char* name (void) {return "BoidsCuPP inkl. simulate inkl. update with GPU-grid V1";}
		...
		preprocess_grid (get_preprocess_grid_kernel(), dim3(1), dim3(threads_per_block)),
		...
		virtual void update (const float /*currentTime*/, const float elapsedTime) {
			...
			preprocess_grid(cupp_device, positions_, grid_, flock_size);
