...
#include "ds/dyn_grid_clever_v2.h"
...
class BoidsPlugIn : public PlugIn {
	private:
		...
		ds::dyn_grid_clever_v2 grid_;

		int think_cycle_;
	public:

		const char* name (void) {return "BoidsCuPP inkl. simulate inkl. update with dyn_grid + clever V2";}
		float selectionOrderSortKey (void) {return 0.03f;}

		BoidsPlugIn() :
		find_neighbours(get_find_neighbours_simulate_dyn_grid_clever_v2_kernel(), dim3 (threads_per_block/threads_per_block), dim3 (threads_per_block)),
		...
		virtual void update (const float /*currentTime*/, const float elapsedTime) {
			...
			for (std::size_t i=0; i < positions_.size(); ++i) {
				grid_.insert (i, positions_[i].get().transform(cupp_device));
			}
