...
class BoidsPlugIn : public PlugIn {
	private:
		...
		ds::dyn_grid_clever grid_;

		int think_cycle_;
	public:

		const char* name (void) {return "BoidsCuPP inkl. simulate inkl. update with dyn_grid + clever";}
		float selectionOrderSortKey (void) {return 0.03f;}

		BoidsPlugIn() :
		find_neighbours(get_find_neighbours_simulate_dyn_grid_clever_kernel(), dim3 (threads_per_block/threads_per_block), dim3 (threads_per_block)),
		...
		virtual void update (const float /*currentTime*/, const float elapsedTime) {
			...
			//#pragma omp parallel num_threads(4)
			{
				...
				for (std::size_t i=0; i < positions_.size(); ++i) {
					grid_.insert (i, i, positions_[i].get().transform(cupp_device));
				}
				
				
				//#pragma omp master
				grid_.order(threads_per_block);
				
			}
			

// 			std::cout << "AAAA: " << grid_.size() << std::endl;
			
			find_neighbours.set_grid_dim (grid_.size());

			resume_simulation_stop_watch();

			find_neighbours (cupp_device, grid_, positions_, forwards_, steering_result_);
