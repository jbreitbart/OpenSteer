...
class BoidsPlugIn : public PlugIn {
	...
	public:
		...
		const char* name (void) {return "BoidsCuPP inkl. simulate inkl. update with grid V1";}
		float selectionOrderSortKey (void) {return 0.03f;}

		BoidsPlugIn() :
		find_neighbours(get_find_neighbours_simulate_frequency_grid_kernel(), dim3 (threads_per_block/threads_per_block), dim3 (threads_per_block)),
		update_flock (get_update_kernel(), dim3 (threads_per_block/threads_per_block), dim3 (threads_per_block)),
		grid_(boid_worldRadius, grid_size/*, get_grid_index_symbol()*/)
		{}
		...
		virtual void update (const float /*currentTime*/, const float elapsedTime) {
			...
			if (flock_size != steering_result_.size()) {
				...
				find_neighbours.set_grid_dim (flock_size/threads_per_block);
			...
			for (std::size_t i=0; i < positions_.size(); ++i) {
				grid_.insert (i, positions_[i]);
			}
			

			resume_simulation_stop_watch();
			find_neighbours (cupp_device, 0, grid_, positions_, forwards_, steering_result_);
