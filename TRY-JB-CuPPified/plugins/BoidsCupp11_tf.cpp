...
class BoidsPlugIn : public PlugIn {
	private:
		...
		ds::grid<int> rest_grid_;

		int think_cycle_;
	public:

		const char* name (void) {return "BoidsCuPP inkl. simulate inkl. update with dyn_grid + clever & grid V1 with think frequency";}
		...
		rest_grid_(boid_worldRadius, grid_size, get_grid_index_symbol_mix())
		{}
		...
		virtual void update (const float /*currentTime*/, const float elapsedTime) {
			...
			for (std::size_t i=0; i < think_lower_boid_index; ++i) {
				rest_grid_.insert (i, positions_[i]);
			}

			for (std::size_t i=think_lower_boid_index; i < think_higher_boid_index ; ++i) {
				think_grid_.insert (i, positions_[i].get().transform(cupp_device));
			}
