...
class BoidsPlugIn : public PlugIn {
	private:
		...
		ds::dyn_grid_clever think_grid_;
		ds::dyn_grid_clever rest_grid_;

		int think_cycle_;
	public:

		const char* name (void) {return "BoidsCuPP inkl. simulate inkl. update with dyn_grid + clever with think frequency";}
		float selectionOrderSortKey (void) {return 0.03f;}

		BoidsPlugIn() :
		find_neighbours(get_find_neighbours_simulate_dyn_grid_clever_tf_kernel(), dim3 (threads_per_block/threads_per_block), dim3 (threads_per_block)),
		...
		virtual void update (const float /*currentTime*/, const float elapsedTime) {
			...
			unsigned int const thinking_boid_count_per_frame = static_cast< int >( std::ceil( static_cast< float >( flock_size ) / static_cast< float >( think_frequency ) ) );
			unsigned int const think_lower_boid_index = think_cycle_ * thinking_boid_count_per_frame;
			unsigned int const think_higher_boid_index = kapaga::clamp_high( think_lower_boid_index + thinking_boid_count_per_frame, flock_size );
			...
			think_grid_.resize(think_higher_boid_index - think_lower_boid_index);
			rest_grid_.resize(think_lower_boid_index + positions_.size() - think_higher_boid_index);

			
			//#pragma omp parallel num_threads(1)
			{
				//#pragma omp for schedule(static) nowait
				for (std::size_t i=0; i < think_lower_boid_index; ++i) {
					OpenSteer::Vec3 temp2 = positions_[i].get();
					const OpenSteer::deviceT::Vec3 temp = temp2.transform(cupp_device);
					rest_grid_.insert (i, i, temp);
				}
	
				//#pragma omp for schedule(static) nowait
				for (std::size_t i=think_lower_boid_index; i < think_higher_boid_index ; ++i) {
					think_grid_.insert (i-think_lower_boid_index, i, positions_[i].get().transform(cupp_device));
				}
				
				//#pragma omp for schedule(static) nowait
				for (std::size_t i=think_higher_boid_index; i < positions_.size(); ++i) {
					rest_grid_.insert (i-think_higher_boid_index+think_lower_boid_index, i, positions_[i].get().transform(cupp_device));
				}
				
				//#pragma omp master
				rest_grid_.order(threads_per_block);
				
				//#pragma omp master
				think_grid_.order(threads_per_block);
			}
			...
			find_neighbours.set_grid_dim (think_grid_.size());

			find_neighbours (cupp_device, think_grid_, rest_grid_, positions_, forwards_, steering_result_);
