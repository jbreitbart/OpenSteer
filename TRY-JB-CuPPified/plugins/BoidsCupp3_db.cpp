...
class BoidsPlugIn : public PlugIn {
	...
	public:
		...
		const char* name (void) {return "BoidsCuPP inkl. simulate inkl. update with double buffering";}
		float selectionOrderSortKey (void) {return 0.03f;}

		BoidsPlugIn() :
		find_neighbours(get_find_neighbours_simulate_kernel(), dim3 (threads_per_block/threads_per_block), dim3 (threads_per_block)),
		...
		virtual void update (const float /*currentTime*/, const float elapsedTime) {
			...
			if (flock_size != steering_result_.size()) {
				...
				find_neighbours.set_grid_dim (grid_dim);
				
				update_flock.set_block_dim (block_dim);
				update_flock.set_grid_dim (grid_dim);
			}
		...
		virtual void redraw (const float /*currentTime*/, const float /*elapsedTime*/) {
			...
			for (std::size_t i=0; i<render_position_[render_position_index_].size(); ++i) {
				renderFeeder() -> render (render_position_[render_position_index_][i], boidGraphicsId_);
			}
