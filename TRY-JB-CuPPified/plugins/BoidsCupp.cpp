...
class Boid {
	public:
		 ...
		 */
		Vec3 simulate(const AVGroup &neighbours ) {
			...
			// they need forward() and position() of the neighbours
...
namespace Graphics {
template <>
class DrawVehicle<B1::Boid> : public std::unary_function< const B1::Boid&, void > {
	public:
		...
		void operator() ( B1::Boid const &_vehicle ) {
...
class BoidsPlugIn : public PlugIn {
	private:
		...
		cupp::vector< int > find_neighbours_result_;

		
	public:

		const char* name (void) {return "BoidsCuPP";}
		...
		void neighbor_search(const int flock_size) {
			static const float maxRadius = maxXXX (separationRadius,
						               maxXXX (alignmentRadius, cohesionRadius)
						              );

			static const float maxRadius2 = maxRadius * maxRadius;
			
			dim3 block_dim(threads_per_block);
			dim3 grid_dim(flock_size/block_dim.x);
			cupp::kernel find_neighbours(get_find_neighbours_kernel(), grid_dim, block_dim);
			
			find_neighbours (cupp_device, positions_, maxRadius2, find_neighbours_result_);
		...
		virtual void update (const float /*currentTime*/, const float elapsedTime) {
			...
			if ((flock_size*neighbour_size_max) != find_neighbours_result_.size()) {
				find_neighbours_result_.resize(flock_size*neighbour_size_max, -1);
			}

			neighbor_search(flock_size);

			// get the results form the device
			// this should NOT be done in parallel region on the CPU
			// as cupp::vector is NOT thread safe
			find_neighbours_result_.update_host();
			
			#pragma omp parallel default( shared )
			{
				std::vector<Boid*> neighbours(neighbour_size_max);
				// update flock simulation for each boid
				#pragma omp for schedule(dynamic)
				for( int i = 0; i < flock_size; ++i ) {
					for (std::size_t j=0; j<neighbour_size_max; ++j) {
						if (find_neighbours_result_[i*neighbour_size_max + j] != -1) {
							neighbours[j] = &flock[find_neighbours_result_[i*neighbour_size_max + j]];
						} else {
							neighbours[j] = 0;
							break;
						}
					...
					// bknafla: separate update and simulation to adapt to parallelization.
					simulatedSteeringForces[ i ] = flock[ i ].simulate(neighbours);
